...
//                ->setCellValue("G".$j,date("Y-m-d H:i:s",$val['addtime']));
    ...
    public function excelimport_work($filename,$sheet="Sheet1",$uid){
        $objPHPExcel = new \PHPExcel();
        $fileType=\PHPExcel_IOFactory::identify($filename);//自动获取文件的类型提供给phpecxel用
        $objReader=\PHPExcel_IOFactory::createReader($fileType);//获取文件读取操作对象
        $sheetName=$sheet;//指定sheet
        ...
        foreach ($objPHPExcel->getWorksheetIterator() as $val){//循环取sheet
            ...
            foreach ($val->getRowIterator() as $row){//逐行处理
                ...
                foreach ($row->getCellIterator() as $key => $cell){//逐列读取
                    ...
                    $data = $cell->getValue();//获取单元格数据
                    if($key==0){$rs['name']=$data?$data:'';}
                    if($key==1){$rs['gender']=$data?$data:'';}
                    if($key==2){$rs['age']=$data?$data:'';}
                    if($key==3){$rs['worker']=$data?$data:'';}
                    if($key==4){$rs['tel']=$data?$data:0;}
                    if($key==5){$rs['id_card']=$data?$data:0;}
                }
                $rs['simg'] = '/Public/upfile/touxiang.jpg';
                $rs['bid'] = $uid;
                $rs['proid'] = session('admin')['proid'];
