...
class Rongyun{
     ...
     * @return json|xml
     */
    public function messageGroupPublish($fromUserId, $toGroupId = array(), $content,$objectName = 'RC:InfoNtf' , $pushContent='', $pushData = '') {
        try{
            if(empty($fromUserId))
                throw new Exception('发送人用户 Id 不能为空');
            if(empty($toGroupId))
     ...
     * @return json|xml
     */
    public function messageSystemPublish($fromUserId,$toUserId = array(),$content,$objectName = 'RC:TxtMsg',$pushContent='',$pushData = '') {
        try{
            if(empty($fromUserId))
                throw new Exception('发送人用户 Id 不能为空');
            if(empty($toUserId))
