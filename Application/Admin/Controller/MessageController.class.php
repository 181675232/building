...
//            }
            ...
            foreach ($data as $key=>$val){
                $data[$key]['username'] = $admin->where("id = '{$val['user_id']}'")->getField('username');
            }
...
//            $data = $table->field('id,title')->where($where)->select();
    ...
    public function getone() {
        if (IS_AJAX) {
            $table = M('Message');
            $where['id'] = I('get.id');
            $table->where("id = '{$where['id']}'")->setField('state',2);
            $object = $table->field('*')
                ->where($where)->find();
            $this->assign($object);
    ...
    public function export() {
        ...
        /*--------post参数--------*/
        $keywords = I('post.Message_search_keywords');
        if (I('post.Message_search_date_from')) $starttime = strtotime(I('post.Message_search_date_from'));
        if (I('post.Message_search_date_to')) $stoptime = strtotime(I('post.Message_search_date_to').' 23:59:59');
        $datetype = I('post.Message_search_date') ? I('post.Message_search_date') : 'addtime';
        /*--------post参数--------*/
...
//        }
        ...
        $execl->excel_Message($data,date('YmdHis',time()));
    }
