...
var tool = {
    search : function () {
        if ($('#tool').form('validate')) {
            $('#post-info').datagrid('load', {
                name: $.trim($('input[name="keywords"]').val()),
                date: $('input[name="datetype"]').val(),
                date_from: $('input[name="starttime"]').val(),
                date_to: $('input[name="stoptime"]').val()
            });
        }else {
            $('#search-date').combobox('showPanel');
        }
    ...
    remove : function () {
        var rows = $('#post-info').datagrid('getSelections');
        if (rows.length > 0) {
            $.messager.confirm('确认操作', '您真的要删除所选的<strong>' + rows.length + '</strong>条记录吗？', function (flag) {
                if (flag) {
                    ...
                    $.ajax({
                        ...
                        beforeSend : function () {
                            $('#post-info').datagrid('loading');
                        },
                        success : function(data, response, status) {
                            if (data) {
                                $('#post-info').datagrid('loaded');
    ...
    redo : function () {
        $('#post-info').datagrid('unselectAll');
    },
    reload : function () {
        $('#post-info').datagrid('reload');
    },
